...
//++
				if ( !(ADLC.status1 & 64) ) qDEBUG("Set tdra.");
//<-
...
//++
				if ( (ADLC.status1 & 64) ) qDEBUG("Clear tdra.");
//<-
...
//++
				if ( !(ADLC.status1 & 64) ) qDEBUG("Set fc.");
//<-
				ADLC.status1 |= 64;			// set Tx Reg Data Available flag.
			} else {
//->				if (DebugEnabled && (ADLC.status1 & 64))
...
//++
				if ( (ADLC.status1 & 64) ) qDEBUG("Clear fc.");
//<-
				ADLC.status1 &= ~64;		// clear Tx Reg Data Available flag.
			}
		}
...
//--	}
//++
	if ( sr2psetemp != ADLC.sr2pse ) pDEBUG(dL"ADLC: PSE SR2Rx priority changed to %d.", dR, ADLC.sr2pse);
//<-
